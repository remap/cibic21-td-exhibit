...
import time
import requests
import CibicObjects.cibic_objects as cibic_objects
import pickle
import os
import CibicObjects.aws_client as aws_client
...
class DataController:
	...
	def Run(self, start=datetime(2022, 6, 1), end=datetime(2022, 8, 24), to_bin=False, path="_exports/pickle_jar/"):
		...
		for k, v in self._User_Map.items():
			v.Match()
		if to_bin:
			self.Dump_to_bin(path+f'{start.strftime("%Y_%m_%d")}-{end.strftime("%Y_%m_%d")}.pickle')
		return
